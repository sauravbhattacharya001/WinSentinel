...
public sealed class FixCommand : ChatCommandBase
{
    ...
    public override bool CanHandle(string input) =>
        input.StartsWith("fix ") || input.StartsWith("/fix ") ||
        input == "fix all" || input == "/fixall" || input == "fix everything";

    public override async Task<ChatResponsePayload> ExecuteAsync(string input, ChatContext context)
    {
        var lower = input.Trim().ToLowerInvariant();

        if (lower == "fix all" || lower == "/fixall" || lower == "fix everything" || lower.StartsWith("fix all"))
            return await HandleFixAllAsync(context);

        var target = lower.StartsWith("/fix ") ? input.Trim()[5..].Trim() : input.Trim()[4..].Trim();
        return await HandleFixAsync(target, context);
    }

    private static async Task<ChatResponsePayload> HandleFixAllAsync(ChatContext context)
    {
    ...
    private static async Task<ChatResponsePayload> HandleFixAsync(string target, ChatContext context)
    {
