...
static int HandlePolicy(CliOptions options)
{
    var ignoreService = new IgnoreRuleService();
    ...
    switch (options.PolicyAction)
    {
        case PolicyAction.Export:
            return HandlePolicyExport(policyManager, options);
        case PolicyAction.Import:
            return HandlePolicyImport(policyManager, options);
        case PolicyAction.Validate:
            return HandlePolicyValidate(policyManager, options);
        case PolicyAction.Diff:
            return HandlePolicyDiff(policyManager, options);
        default:
            Console.ForegroundColor = ConsoleColor.Yellow;
...
static int HandlePolicyExport(PolicyManager pm, CliOptions options)
{
...
static int HandlePolicyImport(PolicyManager pm, CliOptions options)
{
    var filePath = options.PolicyFile ?? "winsentinel-policy.json";
...
static int HandlePolicyValidate(PolicyManager pm, CliOptions options)
{
    var filePath = options.PolicyFile ?? "winsentinel-policy.json";
...
static int HandlePolicyDiff(PolicyManager pm, CliOptions options)
{
    var filePath = options.PolicyFile ?? "winsentinel-policy.json";
