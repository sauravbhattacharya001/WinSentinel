...
public static class LicenseManager
{
    ...
    /// <summary>
    /// Well-known dev-mode public key. LicenseManager accepts envelopes signed
    /// by this key ONLY when <c>WINSENTINEL_DEV_MODE=1</c>. Generated by
    /// <c>tools/IssueTestLicense</c>. NEVER used in production.
    /// </summary>
    public const string DevPublicKeyBase64 = "Ht9bWfK3jRmCxN5vQyE8pLsAuDgZoTk2nJhYeXwVrMc=";

    /// <summary>True when the WINSENTINEL_DEV_MODE environment variable is set to "1".</summary>
    public static bool IsDevMode =>
        string.Equals(Environment.GetEnvironmentVariable("WINSENTINEL_DEV_MODE"), "1", StringComparison.Ordinal);
    ...
    public static LicenseRecord Activate(
        ...
        string? path = null,
        bool allowTestLicense = false)
    {
        string normalizedKey = key?.Trim().ToUpperInvariant() ?? string.Empty;
        if (allowTestLicense && !IsDevMode)
            throw new InvalidOperationException(
                "--allow-test-license requires WINSENTINEL_DEV_MODE=1 environment variable.");

        if (!allowTestLicense && !TryNormalizeKey(key, out normalizedKey))
            throw new ArgumentException($"Invalid license key format: '{key}'. Expected WSP-XXXX-XXXX-XXXX.", nameof(key));
        else if (allowTestLicense)
            TryNormalizeKey(key, out normalizedKey); // best-effort normalize, don't reject
